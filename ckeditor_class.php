...
class ckeditor_wordpress {
	...
	public function file_editor_js() {
		// Codepress support removed (deprecated, unused since WP 3.0.0).
	}

	public function remove_tinymce() {
		// No-op: legacy pre-3.3 TinyMCE hook (wp_tiny_mce) no longer
		// exists in WordPress 3.3+, so this is intentionally empty.
	}

	public function add_post_js() {
		// No-op: legacy pre-3.3 editor hooks (wp_tiny_mce,
		// wp_print_editor_js, wp_preload_dialogs) no longer exist;
		// the modern _WP_Editors class handles this, so this
		// compatibility shim is unnecessary on WordPress 3.3+.
	}
...
final class _WP_Editors {
	...
	public static function editor($content, $editor_id, $settings = array()) {
		...
		if ($set['quicktags'] && $set['tinymce']) {
			...
			if (function_exists('format_for_editor')) {
				// WP 4.3+: format_for_editor() replaces the deprecated
				// wp_htmledit_pre() and wp_richedit_pre() functions.
				add_filter('the_editor_content', 'format_for_editor', 10, 2);
			} elseif ('html' == wp_default_editor()) {
				// Back-compat for WordPress installs older than 4.3.
	...
	/**
	 * WP 4.8+ core added this method (used by the Customizer/Widgets Text
	 * editor to lazy-load the editor after page load). This class must
	 * provide it too, otherwise code that expects the real WordPress
	 * _WP_Editors API (e.g. wp_enqueue_editor() in general-template.php)
	 * hits a fatal "Call to undefined method" error, as seen when opening
	 * Theme > Customize (WP_Widget_Text::enqueue_admin_scripts()).
	 *
	 * Reimplemented here (rather than deferring to core) so CKEditor
	 * keeps replacing the default editor everywhere, including this
	 * lazy-load path, instead of silently falling back to TinyMCE.
	 */
	public static function enqueue_default_editor() {
		...
		/** This action is documented in wp-includes/class-wp-editor.php */
